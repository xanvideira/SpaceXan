...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        total_success = spacex_df[spacex_df['class'] == 1].groupby('Launch Site')['class'].count()
        labels = total_success.index
        values = total_success.values
        fig = px.pie(
            names=labels,
            values=values,
